...
> 源代码在 [nop-chaos](https://gitee.com/canonical-entropy/nop-chaos) 项目中。
...
```xml title="nop-demo-web/src/main/resources/_vfs/nop/demo/auth/nop-demo.action-auth.xml"
    <site id="main">
        ...
        <resource id="staff-management"
                  icon="ion:grid-outline" resourceType="TOPM"
                  displayName="人员管理" routePath="/staff-management"
                  orderNo="10010" component="layouts/default/index">
            <children>
                <resource id="Employee-main" orderNo="10000"/>
            </children>
        </resource>
...
迁移二级菜单，只需要在新的一级菜单中直接引入要迁移的菜单资源即可，
默认会将该 `id` 的资源从原菜单中删除，再与新菜单中的资源做合并。

由于 `resource` 是全局性的，所以，在其他新菜单中不能再通过 `x:prototype`
进行继承和合并，只能直接通过 `id` 引入再合并。

...
> 在实现时，可编写工具从 Tailwind 文档中提取并生成语义化的基础样式。
...
<component>
  <template>
    <import name="Button" from="button.xui" />

    <Button>
      <styles>
        <!-- 与 button.xui 中 styles 内的同名样式做合并，
        再应用到其内部的 button 组件节点上 -->
        <btn-add> <new-btn-add /> </btn-add>
      </styles>
    </Button>
  </template>
...
这就要求，在定义组件内部样式时，需要将每个样式视为组件的样式属性，

...
+++ ...
++ template    | 组件的组成结构。通过 name、id 或 x:id 对子节点做差量
...
- 由 `Block`（块）作为组件的容器元素，负责控制显示区域大小、边界、阴影等，
  其对应 HTML 中的 `div` 元素
...
```xml
<component>
  <ui:import name="Button" from="button.comp.xui" />
  ...
  <component name="MyButton" x:extends="button.comp.xui">
    ...
  </component>
</component>
```
...
- 引用其他组件组成其内部结构
- 组件自身不限定其名字，而是在 `import` 时按需指定：`<ui:import name="Button" from="button.comp.xui" />`
...
不可继续拆分其结构的样式，称之为**原子样式**，比如，边距 `margin` 仅包含上下左右的尺寸信息，

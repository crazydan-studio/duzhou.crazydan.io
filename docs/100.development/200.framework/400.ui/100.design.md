...
- 应用状态自上而下单向流动，并可响应式更新组件状态，但组件只能变更其内部状态且响应式，
  若要更新应用状态，则必须通过消息实现
- 统一术语、统一单位、统一尺寸
- UI 层与逻辑处理层是完全不同的层：前者是将数据以可视化形式展现，并通过事件接受用户交互，
  再根据交互对数据的影响而做出响应，仅仅是对数据本身及其变化的展现，并不关心数据来源及其变化方式；
  而后者则主要是对数据进行业务相关的逻辑处理，涉及对数据的增删改以及结构变换等操作
...
- 可引用外部定义的数据类型
- 页面路由
  - 在进入或退回时，均支持参数传递（退回时传递的额外数据，其进入时的参数保持不变），以便于交换页面数据
...
- 仅针对支持输入的组件：`xui:validator="var-name"`
- 可设置校验结果提示的显示位置、显示形式、显示样式等
  - 位置：在相关输入组件的四周、在某个位置集中显示、
  - 形式：提示气泡、告警消息
  - 样式：输入组件标签及其边框变红、提示文本颜色、

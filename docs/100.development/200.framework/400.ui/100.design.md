...
- `*.xui` 定义设计时结构，直接处理 XNode 结构，保证可差量、DSL 结构清晰、语义完整
- `*.rt.xui` 定义运行时结构，需要生成 class 模型，保证代码逻辑清晰，意义明确
- 在 `*.rt.xui` 中通过 `x:gen-extends` 函数，将指定路径的 xui 形式变换为运行时结构
  - `<runtime:GenRuntime xpl:lib="xxx" path="xxx.xui" />`
  - `*.rt.xui` 与变换函数定义在 schema 模块中
  - 在 runtime 模块生成 rt 的 class 模型
...
在引用组件时所指定的样式将与该组件自身定义的样式做合并，例如，定义了如下结构的 `Button` 组件：
...
<component>
  ...
  <styles x:extends="base.styles.xui">
    <add-btn size="xui:size" color="xui:color">
      <font ... />
      <background ... />
      <border ... />
    </add-btn>
...
对组件样式属性的局部修改，类似于组件模板中的 `slot` 机制，也就是组件 `Button`
中会向 `<add-btn />` 的传参数值，而在 `form.xui` 中定制 `<add-btn />`
则相当于重载 `Button` 的 `<add-btn />` 的函数实现。但，样式函数的重载仅针对复合组件，
在原子组件这一层，则只能是样式函数的调用。

...
- `react`、`typescript` 插件
- `tsconfig.json` 配置
- 参考 [embed.tsx](https://github.com/baidu/amis/blob/master/examples/embed.tsx)
  构建 AMIS 渲染器

以 AMIS React 集成开发有如下好处：

- 可拆分打包 js，避免单个文件过大，并可以实现按需加载依赖
- 避免引入 vue 等其他框架代码，统一开发的技术栈，降低混合开发造成的负面影响

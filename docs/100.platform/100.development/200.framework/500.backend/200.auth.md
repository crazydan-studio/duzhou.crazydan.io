...
| `title` | 前端站点主标题 |  |
| `subTitle` | 前端站点副标题 |  |
| `renderer.script` | 前端站点渲染引擎 js 地址 | 指定 `name` 可便于定制化重载，`name` 为 `renderer` 的脚本为主脚本，其余的为依赖脚本。注：声明顺序为其加载顺序 |
| `renderer.style` | 前端站点渲染引擎的 css 地址 | 指定 `name` 可便于定制化重载。注：声明顺序为其加载顺序 |
